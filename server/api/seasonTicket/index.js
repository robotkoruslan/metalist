...
router.get('/season-tickets', auth.hasRole('cashier' || 'api'), controller.getSeasonTickets);
router.get('/block-row', auth.hasRole('admin'), controller.getBlocks);

router.post('/extend', auth.hasRole('cashier' || 'api'), controller.extendSeasonTicket);
...
router.post('/registration/:slug', auth.hasRole('cashier' || 'api'), controller.confirmSeasonTicket);
